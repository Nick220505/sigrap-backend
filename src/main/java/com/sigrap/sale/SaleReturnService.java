 ...
 *
 * <p>
 * This service handles the business logic for creating, retrieving, updating,
 * and deleting sales returns. It interacts with various repositories to manage
 * sales return data, product stock adjustments, and related entities.
 * </p>
 */
@Service
...
public class SaleReturnService {
   ...
   * <ul>
   * <li>Validates the existence of the original sale, customer, and
   * employee.</li>
   * <li>Ensures the customer making the return is the same as in the original
   * sale.</li>
   * <li>Persists the new {@link SaleReturn} entity.</li>
   * <li>Processes each item in the return, adjusting product stock levels by
   * increasing them.</li>
   * </ul>
   * </p>
   *
   ...
   * @throws EntityNotFoundException  if the original sale, customer, or employee
   *                                  is not found.
   * @throws IllegalArgumentException if the return customer does not match the
   *                                  original sale customer,
   *                                  or if any returned item is invalid (e.g.,
   *                                  product not in original sale,
   *                                  quantity to return exceeds quantity
   *                                  purchased).
   */
  @Transactional
  @Auditable(action = "CREATE", entity = "SALE_RETURN", captureDetails = true)
   ...
   *
   * <p>
   * For each item in the return data:
   * <ul>
   * <li>Validates the existence of the product.</li>
   * <li>Ensures the product was part of the original sale.</li>
   * <li>Validates that the quantity being returned does not exceed the quantity
   * purchased in the original sale.</li>
   * <li>Increases the stock of the product by the quantity returned.</li>
   * <li>Persists the {@link SaleReturnItem} entity.</li>
   * </ul>
   * </p>
   *
   * @param saleReturn   The parent {@link SaleReturn} entity to associate items
   *                     with.
   * @param originalSale The {@link Sale} entity from which items are being
   *                     returned, for validation.
   * @param itemsData    A list of {@link SaleReturnItemData} DTOs representing
   *                     the items to be processed.
   * @throws EntityNotFoundException  if a product specified in an item is not
   *                                  found.
   * @throws IllegalArgumentException if a product was not in the original sale,
   *                                  or if the return quantity is invalid.
   */
  private void processReturnItems(
    ...
    for (SaleReturnItemData itemData : itemsData) {
      ...
      Optional<SaleItem> originalSaleItemOpt = originalSale
          .getItems()
          .stream()
          .filter(originalItem -> originalItem.getProduct().getId().equals(product.getId()))
          .findFirst();
   ...
   *
   * @return A list of {@link SaleReturnInfo} DTOs, each representing a sales
   *         return.
   ...
   * @throws EntityNotFoundException if no sales return is found with the given
   *                                 ID.
   */
  @Transactional(readOnly = true)
   ...
   *
   * @param originalSaleId The ID of the original sale for which to retrieve
   *                       returns.
   * @return A list of {@link SaleReturnInfo} DTOs related to the specified
   *         original sale.
   *         Returns an empty list if no returns are found for the given original
   *         sale ID.
   * @throws EntityNotFoundException if the original sale with the given ID is not
   *                                 found.
   */
  @Transactional(readOnly = true)
   ...
   *
   * <p>
   * This method also reverts the stock adjustments made when the return was
   * created.
   * For each item in the deleted return, the product stock is decreased by the
   * quantity
   * that was returned (effectively undoing the stock increase from the return).
   * </p>
   *
   ...
   */
  @Transactional
  @Auditable(action = "DELETE", entity = "SALE_RETURN", entityIdParam = "id")
  public void delete(Integer id) {
    SaleReturn saleReturn = saleReturnRepository
        .findById(id)
        .orElseThrow(() -> new EntityNotFoundException("Sale return not found with id: " + id));
   ...
   * Validates all IDs exist before performing the deletion and adjusts product
   * stock accordingly.
   *
   ...
   */
  @Transactional
  @Auditable(action = "BATCH_DELETE", entity = "SALE_RETURN", entityIdParam = "ids")
   ...
   * This method performs several operations:
   * <ul>
   * <li>Validates the existence of the sales return, original sale, customer, and
   * employee.</li>
   * <li>Ensures that the original sale and customer are not changed during the
   * update.</li>
   * <li>Updates the employee processing the return and the reason for the
   * return.</li>
   * <li>Manages the list of returned items:
   * <ul>
   * <li>Items not present in the new data are removed, and their stock impact is
   * reversed (stock decreased).</li>
   * <li>Existing items are updated (quantity, price), and stock is adjusted for
   * the difference.</li>
   * <li>New items are added, and their stock impact is applied (stock
   * increased).</li>
   * </ul>
   * </li>
   * <li>Recalculates and updates the total return amount based on the modified
   * items.</li>
   * <li>Persists the changes to the {@link SaleReturn} and its associated
   * {@link SaleReturnItem}s.</li>
   * </ul>
   * </p>
   *
   * @param id             The ID of the sales return to update.
   * @param saleReturnData The {@link SaleReturnData} DTO containing the updated
   *                       information.
   * @return A {@link SaleReturnInfo} DTO representing the updated sales return.
   * @throws EntityNotFoundException  if the sales return, original sale,
   *                                  customer, employee, or any product in items
   *                                  is not found.
   * @throws IllegalArgumentException if an attempt is made to change the original
   *                                  sale or customer,
   *                                  or if any item validation fails (e.g.,
   *                                  returning more than purchased).
   */
  @Transactional
  @Auditable(action = "UPDATE", entity = "SALE_RETURN", entityIdParam = "id", captureDetails = true)
  public SaleReturnInfo update(Integer id, SaleReturnData saleReturnData) {
    SaleReturn existingSaleReturn = saleReturnRepository
        .findById(id)
        .orElseThrow(() -> new EntityNotFoundException("SaleReturn not found with ID: " + id));
    ...
    if (!existingSaleReturn.getOriginalSale().getId().equals(originalSale.getId())) {
      throw new IllegalArgumentException(
   ...
   *
   * <p>
   * This method synchronizes the persisted {@link SaleReturnItem}s with the
   * provided
   * list of {@link SaleReturnItemData}. It handles:
   * <ul>
   * <li><strong>Item Removal</strong>: If an existing item is not in
   * {@code newItemsData},
   * it's deleted, and its quantity is subtracted from the product's stock
   * (reversing the initial return stock adjustment for that item).</li>
   * <li><strong>Item Modification</strong>: If an item exists in both current and
   * new data,
   * its details (quantity, price, subtotal) are updated. The product stock is
   * adjusted by the difference
   * between the new and old quantities.</li>
   * <li><strong>Item Addition</strong>: If an item in {@code newItemsData} is not
   * an existing item,
   ...
   * @param originalSale The {@link Sale} entity from which items were originally
   *                     returned, for validation.
   * @param newItemsData A list of {@link SaleReturnItemData} DTOs representing
   *                     the desired state of items for the return.
   * @throws EntityNotFoundException  if any product specified in
   *                                  {@code newItemsData} is not found.
   * @throws IllegalArgumentException if any validation fails, such as returning
   *                                  more items than purchased or referencing a
   *                                  product not in the original sale.
   */
  private void updateReturnItems(
      SaleReturn saleReturn,
      Sale originalSale,
      List<SaleReturnItemData> newItemsData) {
    List<SaleReturnItem> existingItems = saleReturnItemRepository.findBySaleReturn(saleReturn);
    ...
    for (SaleReturnItemData itemData : newItemsData) {
      ...
      SaleItem originalSaleItem = originalSale
          .getItems()
          .stream()
          .filter(origItem -> origItem.getProduct().getId().equals(product.getId()))
          .findFirst()
      ...
      if (existingItemOpt.isPresent()) {
        ...
        int quantityDifference = itemData.getQuantity() - existingItem.getQuantity();
   ...
   * @return The sum of subtotals of all items in the list as a
   *         {@link BigDecimal}.
