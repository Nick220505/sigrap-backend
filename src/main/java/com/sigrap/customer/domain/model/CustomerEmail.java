package com.sigrap.customer.domain.model;

import java.util.regex.Pattern;

/**
 * Value object representing a customer email address.
 * Immutable and self-validating.
 */
public record CustomerEmail(String value) {
    
    private static final Pattern EMAIL_PATTERN = Pattern.compile(
        "^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$"
    );
    
    /**
     * Compact constructor with validation.
     * Ensures the email is valid (not blank, valid format, and within length limits).
     *
     * @throws IllegalArgumentException if the value is null, blank, invalid format, or exceeds maximum length
     */
    public CustomerEmail {
        if (value == null || value.isBlank()) {
            throw new IllegalArgumentException("Customer email cannot be blank");
        }
        if (value.length() > 255) {
            throw new IllegalArgumentException("Customer email cannot exceed 255 characters");
        }
        if (!EMAIL_PATTERN.matcher(value).matches()) {
            throw new IllegalArgumentException("Customer email must be a valid email address");
        }
    }
}
