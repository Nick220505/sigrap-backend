...
public class AttendanceService {
   ...
   * @param timestamp The clock-in timestamp, or null to use current time in
   *                  Bogota, Colombia
   * @return AttendanceInfo containing the created attendance record
   * @throws EntityNotFoundException if the user is not found
   * @throws IllegalStateException   if the user already has an active attendance
   *                                 record
   */
  @Transactional
  @Auditable(action = "CLOCK_IN", entity = "ATTENDANCE", entityIdParam = "userId", captureDetails = true)
   ...
   * @param timestamp    The clock-out timestamp, or null to use current time in
   *                     Bogota, Colombia
   * @return AttendanceInfo containing the updated attendance record
   * @throws EntityNotFoundException if the attendance record is not found
   */
  @Transactional
  @Auditable(action = "CLOCK_OUT", entity = "ATTENDANCE", entityIdParam = "attendanceId", captureDetails = true)
   ...
   * @return EARLY_DEPARTURE if leaving before standard end time - threshold,
   *         PRESENT otherwise
   */
  private AttendanceStatus determineClockOutStatus(LocalTime clockOutTime) {
    if (clockOutTime.isBefore(
  ...
  public List<AttendanceInfo> generateAttendanceReport(
    ...
    List<Attendance> attendances = attendanceRepository.findByUserIdAndDateBetween(
        userId,
        startDate,
        endDate);
    return attendanceMapper.toInfoList(attendances);
   ...
   */
  @Transactional
  @Auditable(action = "UPDATE", entity = "ATTENDANCE", entityIdParam = "id", captureDetails = true)
