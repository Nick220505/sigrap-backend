...
class SaleExportControllerTest extends BaseIntegrationTest {
    ...
    void generateDailySalesReport_withAUTOExportPath_returnsFileContent()
            throws Exception {
        String reportContent = "CUSTOMER_ID|SALE_DATE|TOTAL_AMOUNT|TOTAL_AMOUNT_WITH_TAX\n" +
                "123456789|15/06/2023|100|119";

        String expectedFilename = "STATIONERY020_" + formattedDate + ".txt";

        when(
                saleExportService.generateDailySalesReportContent(eq(testDate))).thenReturn(reportContent);
    ...
    void generateDailySalesReport_withCustomExportPath_returnsFilePath()
        ...
        String generatedFilePath = exportPath + "/STATIONERY020_" + formattedDate + ".txt";

        when(
                saleExportService.generateDailySalesReport(eq(testDate), eq(exportPath))).thenReturn(generatedFilePath);
    ...
    void generateDailySalesReport_withNoDate_usesTodaysDate() throws Exception {
        String exportPath = "/custom/path";
        String generatedFilePath = "/custom/path/STATIONERY020_some-date.txt";
