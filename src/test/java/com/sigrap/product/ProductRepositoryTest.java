...
class ProductRepositoryTest {
  ...
  void shouldSaveProduct() {
    ...
    assertThat(savedProduct.getCostPrice()).isEqualByComparingTo(
  ...
  void shouldFindProductById() {
    ...
    Optional<Product> foundProduct = productRepository.findById(
      savedProduct.getId()
    );

    assertThat(foundProduct).isPresent();
    assertThat(foundProduct.get().getName()).isEqualTo("Test Product");
    assertThat(foundProduct.get().getCostPrice()).isEqualByComparingTo(
      new BigDecimal("10.00")
    );
  }
