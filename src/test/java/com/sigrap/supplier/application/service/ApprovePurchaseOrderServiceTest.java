...
class ApprovePurchaseOrderServiceTest {
    ...
    @InjectMocks
    private ApprovePurchaseOrderService approvePurchaseOrderService;
    
    @Test
    void shouldApprovePendingPurchaseOrder() {
        // Given
        ...
        // When
        PurchaseOrder result = approvePurchaseOrderService.approve(id);
    ...
    void shouldThrowExceptionWhenPurchaseOrderNotFound() {
        ...
        IllegalArgumentException exception = assertThrows(
            ...
            () -> approvePurchaseOrderService.approve(id)
        );
    ...
    @Test
    void shouldThrowExceptionWhenPurchaseOrderIsNotPending() {
        // Given
        PurchaseOrderId id = new PurchaseOrderId(1L);
        PurchaseOrder purchaseOrder = new PurchaseOrder(
            id,
        ...
        assertTrue(exception.getMessage().contains("Can only approve pending"));
        verify(purchaseOrderRepository).findById(id);
        verify(purchaseOrderRepository, never()).save(any());
    }
    ...
    void shouldThrowExceptionWhenPurchaseOrderIsReceived() {
        ...
        // When & Then
        assertThrows(IllegalStateException.class, () -> approvePurchaseOrderService.approve(id));
        verify(purchaseOrderRepository, never()).save(any());
    }
